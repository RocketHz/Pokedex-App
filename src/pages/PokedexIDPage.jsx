...
const PokedexIDPage = () => {
  ...
  return (
    <div className="pokeID_container">
      <div className="pokeID_header">
        ...
        <div className="pokeID_header_content">
          <img
        ...
        <div className="pokeID_content_type">
          ...
          <ul className="pokeID_type">
            {pokemon?.types.map((typeInfo) => {
              ...
              return (
                <li className={`${typeColor}`} key={typeInfo.type.url}>
                  {typeInfo.type.name}
                </li>
              );
            })}
        ...
        <div className='pokeID_content_ability'>
          <h3>Abilities</h3>
          <ul className="pokeID_type">
  ...
  {pokemon?.stats.map((statInfo) => {
    const percentage = statInfo.base_stat / 100;
    const color = statInfo.base_stat < 25 ? 'red' : statInfo.base_stat < 50 ? 'yellow' : 'green';

    return (
      <ul className="pokeID_stats" key={statInfo.stat.url}>
        <span className="poke_stats_name">{statInfo.stat.name}</span>
        <li className="poke_stats_content">
          <div className="pokeID_stats_bar">
            <span className="poke_stats_base">{statInfo.base_stat}</span>
            <div className="pokeID_stats_bar_color fillbar" style={{ width: `${percentage * 100}%`, backgroundColor: color }}></div>
          </div>
        </li>
      </ul>
    );
